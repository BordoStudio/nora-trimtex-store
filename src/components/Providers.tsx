...
export function Providers({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    void fetch("/api/account/cart").then(async (response) => {
    ...
    return store.subscribe(() => {
      ...
      syncTimer = window.setTimeout(() => { void fetch("/api/account/cart", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ items: safeItems, locale: location.pathname.split("/")[1] || "en" }) }); }, 500);
    });
  }, [store]);
