...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const faq = {
    ...
    de: [["Kann ich Farbmuster bestellen?", "Ja. Wählen Sie die Farbe auf der Produktseite, legen Sie sie in den Warenkorb und senden Sie eine Anfrage."], ["Liefern Sie innerhalb Europas?", "Ja. Lieferzeit, Verfügbarkeit und Versandkosten werden im persönlichen Angebot bestätigt."], ["Wo sehe ich die Großhandelspreise?", "Im Katalog stehen die Kundenpreise. Bestätigte Designer und Partner sehen nach der Anmeldung ihre Preise. Artikel ohne Festpreis werden nach Menge und Lieferbedingungen kalkuliert."]],
    en: [["Can I order colour samples?", "Yes. Select a colour on the product page, add it to the cart and submit an enquiry."], ["Do you supply projects across Europe?", "Yes. Lead time, availability and shipping cost are confirmed in your personal quotation."], ["Where can I see wholesale prices?", "The catalogue shows client prices. Approved designers and partners see their prices after signing in. Articles without a fixed price are quoted by quantity and delivery terms."]],
  }[locale];
  ...
  return <>
    ...
    <section className="hero">
      <div className="hero-copy"><p className="eyebrow">{t.hero.eyebrow}</p><h1>{t.hero.title}</h1><p className="hero-body">{t.hero.body}</p><div className="hero-actions"><Link className="button primary" href={`/${locale}/catalog`}>{t.hero.primary}</Link><Link className="text-link" href={`/${locale}/catalog?category=samples`}>{t.hero.secondary}</Link></div></div>
